...
function _getInitHtml(themesPath, bodyClass, cssPath, cssData) {
	...
	_each(cssPath, function(i, path) {
		if (path) {
			arr.push('<link href="../../kindeditor/' + path + '" rel="stylesheet" />');
		}
	});
	if (cssData) {
...
_extend(KUploadButton, {
	init : function(options) {
		...
		var html = [
			...
			'<form class="ke-upload-area ke-form" method="post" enctype="multipart/form-data" target="' + target + '" action="../../kindeditor/' + url + '">',
...
_plugin('core', function(K) {
	...
	self.beforeSetHtml(function(html) {
		...
		.replace(/<a[^>]*name="([^"]+)"[^>]*>(?:<\/a>)?/ig, function(full) {
			var attrs = _getAttrList(full);
			return '<img class="ke-anchor" src="../../kindeditor/' + self.themesPath + 'common/anchor.gif" data-ke-name="' + escape(attrs.name) + '" />';
		})
